...
def create_submission(model,test_df, test_df_ix, label=None):
    '''
    ...
    submission.to_csv("../data/submission_svm_"+timestr+"_"+label+".csv")
...
if __name__ == '__main__':
    ...
    print("Accuracy on validation set (5K training set)", accuracy)
    
    print('Running training on full sample data')
    clf = train_svc(X_train, Y_train)
    accuracy, predictions = predict_validation_result(clf,X_validate,Y_validate)
    
    print("Accuracy on validation set (80% training set)", accuracy)
    create_submission(clf, test_df,test_df_ix, '80_p_training' )
    ...
    create_submission(clf, test_df,test_df_ix, 'full_training')
